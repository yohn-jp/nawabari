...
import { loadWorktreeProfileCatalog } from "./domain/worktree-profile-catalog.js";
...
function profileSources(cwd: string, base: string | null = null): DomainResult<WorktreeProfileCliSources> {
  try {
    ...
    const revision = defaultGit.run(["rev-parse", "--verify", `${base ?? "HEAD"}^{commit}`], repository.worktreePath);
    // An absent repository catalog is an intentional empty optional source.
    defaultGit.run(["cat-file", "-e", `${revision}:nawabari.profiles.json`], repository.worktreePath);
    const catalog = loadWorktreeProfileCatalog(repository, revision, "nawabari.profiles.json", defaultGit);
    return catalog.ok ? { ok: true, value: { repository: catalog.value } } : catalog;
  } catch {
