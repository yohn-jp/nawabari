import { CGROUPS_V2_ROOT, deriveCgroupScopeName, readCgroupAccounting, type CgroupAccounting, type CgroupFileSystem, type CgroupScope } from "./cgroups-v2.js";
import {
...
export type SessionResourceAccountingExecution = Readonly<{
  readonly record: SessionExecutionRecord;
  /** A previously captured identity observation may be supplied by the caller. */
  readonly observation?: SessionExecutionIdentityObservation;
  /** Injectable scope used by controlled runtimes and hermetic tests. */
...
const NUMERIC_FIELDS = [
  ...
  "memory_current_bytes", "pids_current", "pids_max_events", "memory_oom_kill_events", "memory_max_events",
] as const;

function scopeFor(record: SessionExecutionRecord, root: string): CgroupScope {
...
function emptyAggregate(complete: boolean) {
  return {
    complete,
    cpu_usage_usec: null, cpu_user_usec: null, cpu_system_usec: null, cpu_throttled_usec: null,
    memory_current_bytes: null, memory_peak_bytes: null, pids_current: null, pids_max_events: null,
    memory_oom_kill_events: null, memory_max_events: null,
    cpu_throttled: null, memory_limit_exceeded: null, pids_limit_exceeded: null,
  } as const;
}
...
export function projectSessionResourceAccounting(
  executions: readonly SessionResourceAccountingExecution[],
  options: SessionResourceAccountingOptions = {},
): SessionResourceAccountingProjection {
  const sessionId = options.session_id ?? executions[0]?.record.session_id ?? "";
  const ordered = executions.filter((entry) => entry.record.session_id === sessionId).slice().sort((a, b) =>
  ...
  const entries = selected.map(({ record, observation, scope: suppliedScope }): SessionExecutionResourceEvidence => {
    const observed = observation === undefined ? observeExecutionIdentity(record, { reader: options.identity_reader }) : undefined;
    const identity = observation ?? (observed?.ok === true ? observed.value : undefined);
    if (identity === undefined || !identity.matches || identity.session_id !== sessionId || identity.execution_id !== record.execution_id) {
      return { execution_id: record.execution_id, status: "unavailable", accounting: null, reason: "identity-unverified" };
    }
    const scope = suppliedScope ?? scopeFor(record, options.root ?? CGROUPS_V2_ROOT);
    if (scope.identity.session_id !== record.session_id || scope.identity.execution_id !== record.execution_id || scope.name !== deriveCgroupScopeName(record.cgroup_identity) || scope.path !== `${scope.root}/nawabari/${scope.name}`) {
      return { execution_id: record.execution_id, status: "unavailable", accounting: null, reason: "identity-unverified" };
    }
    try {
      if (!options.filesystem?.statSync(scope.path).isDirectory()) {
        return { execution_id: record.execution_id, status: "unavailable", accounting: null, reason: "cgroup-unavailable" };
      }
    } catch {
      return { execution_id: record.execution_id, status: "unavailable", accounting: null, reason: "cgroup-unavailable" };
    }
    ...
    if (NUMERIC_FIELDS.every((field) => accounting[field] === null)) {
      return { execution_id: record.execution_id, status: "unavailable", accounting: null, reason: "accounting-unreadable" };
    }
    return { execution_id: record.execution_id, status: "available", accounting, reason: null };
  });
  const available = entries.filter((entry): entry is SessionExecutionResourceEvidence & { accounting: CgroupAccounting } => entry.status === "available" && entry.accounting !== null);
  ...
  for (const field of NUMERIC_FIELDS) {
    aggregate[field] = available.length > 0 && available.every((entry) => entry.accounting[field] !== null)
      ? available.reduce((sum, entry) => sum + (entry.accounting[field] as number), 0) : null;
  }
