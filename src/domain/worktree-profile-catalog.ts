...
function error(code: "RUNTIME_PROFILE_INVALID" | "RUNTIME_PROFILE_AMBIGUOUS" | "RUNTIME_PROFILE_MISSING", message: string, details: RecordValue = {}): DomainResult<never> {
...
function profileKey(profile: WorktreeRuntimeProfile): string { return `${profile.id}\u0000${profile.version}`; }
...
export function validateWorktreeProfileCatalog(input: unknown): DomainResult<WorktreeProfileCatalog> {
  ...
  if (!record(value) || !Array.isArray(value.profiles)) return error("RUNTIME_PROFILE_INVALID", "Worktree profile catalog must contain a profiles array.");
  if (value.profiles.length === 0 || value.profiles.length > WORKTREE_PROFILE_CATALOG_MAX_SIZE) return error("RUNTIME_PROFILE_INVALID", "Worktree profile catalog size is outside its bounds.");
  ...
  for (const [index, item] of value.profiles.entries()) {
    ...
    if (!Array.isArray(item.extends) || item.extends.some((parent) => typeof parent !== "string")) return error("RUNTIME_PROFILE_INVALID", `Catalog profile ${index} has invalid extends.`);
    const { extends: parents, ...profileInput } = item;
    const result = validateWorktreeRuntimeProfile(profileInput);
    if (!result.ok) return failure(result.error);
    if (ids.has(result.value.id)) return error("RUNTIME_PROFILE_AMBIGUOUS", `Duplicate worktree profile '${result.value.id}'.`, { profile_id: result.value.id });
    ids.add(result.value.id);
    if (parents.some((parent) => parent === result.value.id)) return error("RUNTIME_PROFILE_AMBIGUOUS", `Profile '${result.value.id}' extends itself.`);
...
export function loadWorktreeProfileCatalog(
  ...
  if (!/^[0-9a-f]{40}$/u.test(revision)) return error("RUNTIME_PROFILE_INVALID", "Catalog revision must be an immutable full commit id.");
  if (path !== WORKTREE_PROFILE_CATALOG_PATH || path.includes("/") || path.includes("\\") || path.includes("..")) return error("RUNTIME_PROFILE_INVALID", "Catalog path is not canonical.");
  try {
    ...
    return error("RUNTIME_PROFILE_MISSING", "The pinned repository catalog could not be read.", { path, revision, cause: cause instanceof Error ? cause.message : String(cause) });
  }
}
...
function mergeProfile(parent: CatalogWorktreeProfile, child: CatalogWorktreeProfile): DomainResult<CatalogWorktreeProfile> {
  ...
  for (const field of fields) {
    ...
    if (a !== b && field !== "materialSelection" && field !== "filesystem") return error("RUNTIME_PROFILE_AMBIGUOUS", `Profile '${child.id}' conflicts with inherited field '${field}'.`);
  }
  const material = { ...child.materialSelection, profiles: [...new Set([...parent.materialSelection.profiles, ...child.materialSelection.profiles])].sort() };
...
export function resolveWorktreeProfile(selection: unknown, catalog: unknown): DomainResult<ResolvedWorktreeRuntimeProfile> {
  const parsedCatalog = validateWorktreeProfileCatalog(catalog);
  if (!parsedCatalog.ok) return failure(parsedCatalog.error);
  if (!record(selection) || typeof selection.profile !== "string") return error("RUNTIME_PROFILE_INVALID", "Profile selection requires one profile id.");
  ...
  const visit = (id: string, depth: number): DomainResult<CatalogWorktreeProfile> => {
    const existing = resolved.get(id); if (existing) return success(existing);
    if (depth > WORKTREE_PROFILE_INHERITANCE_MAX_DEPTH) return error("RUNTIME_PROFILE_INVALID", "Worktree profile inheritance exceeds its maximum depth.");
    const profile = byId.get(id); if (!profile) return error("RUNTIME_PROFILE_MISSING", `Unknown parent profile '${id}'.`, { profile_id: id });
    if (visiting.has(id)) return error("RUNTIME_PROFILE_AMBIGUOUS", `Worktree profile inheritance cycle includes '${id}'.`, { profile_id: id });
    visiting.add(id);
    ...
    for (const parent of (profile as WorktreeRuntimeProfile & { extends?: readonly string[] }).extends ?? []) {
      const parentResult = visit(parent, depth + 1); if (!parentResult.ok) { visiting.delete(id); return parentResult; }
      current = mergeProfile(parentResult.value, current.ok ? current.value : profile);
      if (!current.ok) { visiting.delete(id); return current; }
    }
    visiting.delete(id); resolved.set(id, current.value); return current;
  };
...
export function substituteProfileParameters(profile: unknown, values: WorktreeProfileParameterValues): DomainResult<ResolvedWorktreeRuntimeProfile> {
  const checked = validateWorktreeRuntimeProfile(profile); if (!checked.ok) return failure(checked.error);
  const allowed = new Set(["materialSelection.profiles", "shell.entrypoint"]);
  for (const key of Object.keys(values)) if (!allowed.has(key)) return error("RUNTIME_PROFILE_INVALID", `Unknown profile parameter '${key}'.`);
  ...
  if ("materialSelection.profiles" in values) {
    ...
    if (!Array.isArray(selected) || selected.some((value) => typeof value !== "string")) return error("RUNTIME_PROFILE_INVALID", "materialSelection.profiles requires profile identifiers.");
  ...
  const result = validateWorktreeRuntimeProfile(output); return result.ok ? result : failure(result.error);
}
